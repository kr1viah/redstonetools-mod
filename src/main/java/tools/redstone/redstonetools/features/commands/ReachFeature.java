...
public class ReachFeature {
	...
	private int execute(CommandContext<ServerCommandSource> context, boolean block, boolean entity) throws CommandSyntaxException {
		...
		if (block)
			context.getSource().getServer().getCommandManager().executeWithPrefix(context.getSource(), "/attribute " + playerName + " minecraft:block_interaction_range base set " + reach);
		if (entity)
			context.getSource().getServer().getCommandManager().executeWithPrefix(context.getSource(), "/attribute " + playerName + " minecraft:entity_interaction_range base set " + reach);
		return 0;
	}
	...
	private int reset(CommandContext<ServerCommandSource> context, boolean block, boolean entity) throws CommandSyntaxException {
		String playerName = context.getSource().getPlayerOrThrow().getName().getString();
		if (block)
			context.getSource().getServer().getCommandManager().executeWithPrefix(context.getSource(), "/attribute " + playerName + " minecraft:block_interaction_range base reset");
		if (entity)
			context.getSource().getServer().getCommandManager().executeWithPrefix(context.getSource(), "/attribute " + playerName + " minecraft:entity_interaction_range base reset");
		return 0;
	}
}
